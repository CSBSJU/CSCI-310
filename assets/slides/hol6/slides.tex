...
\begin{document}
  ...
  %{{{1
    ...
    \note[item]<2>{which visualization is correct?}
  %}}}1
  \end{frame}
  ...
  %{{{1
    ...
    \begin{scriptsize}
      ...
      !\tikz[remember picture] \coordinate (a);!int puissance!\tikz[remember picture] \node [] (b) {};
      !(int x,!\tikz[remember picture] \node [] (c){};
      !int n) { 
        /* ... */

        return x+n;!\tikz[remember picture] \coordinate (d);!
      }
      \end{codeblock}
      \begin{tikzpicture}[remember picture, overlay,
          every edge/.append style={
        ...
        \node[above left=.25cm and -.75cm of b] (B) {function name};
        \node [above right=0.20cm and -.75cm of B] (C) {list of formal parameters};
        \node[] at ([shift=({19em,.625ex})]d) (D) {type of \texttt{<expr>} must
          match retury type of function};
        ...
        \draw (D.west) edge ([shift=({.4em,.625ex})]d);
      \end{tikzpicture} 
    \end{scriptsize}
  %}}}1
  \end{frame}
