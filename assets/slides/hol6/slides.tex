...
\begin{document}
  ...
  %{{{1
    \begin{figure}
      ...
      \begin{subfigure}[b]{.30\textwidth}
        ...
        \begin{tikzpicture}[memory=left]
          ...
          \foreach [
            count=\i,
            evaluate=\i as \y using (1.5*(\i-1)),
          ] \l/\v in {j/424,ch/N,ptr1/,ptr2/}
          {
            \node[var=\l] at (0.0,\y) (var\i) {\v};
          }
          % valid pointers
          \foreach \s/\e in {3/1,3/2}
          {
            \draw[ptr]
              (var\s.center) node[dot] {} --
              ([xshift=1em]var\s.east) --
              ([xshift=1em]var\e.east) --
              (var\e.east);
          }
          % null pointers
          \foreach \s in {4}
          {
            \draw[fill]
              (var\s.center) node[dot] {} --
              ([xshift=1em]var\s.east) node[null] {};
          }
        \end{tikzpicture}
        \caption{Fixed location.}
      \end{subfigure}
      ...
      \begin{subfigure}[b]{.30\textwidth}
        \centering
        \begin{tikzpicture}[memory=right]
          % the shadow for the run-time stack
