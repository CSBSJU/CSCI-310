...
\begin{document}
  ...
  %{{{1
    ...
    \note{
      \begin{itemize}
        \item This program would behave exactly the same had this program
          declared \texttt{ch} and \texttt{j} as local variables instead of
          global variables.
      \end{itemize}
    }
  %}}}1
  ...
  %{{{1
    \begin{description}[style=nextline]
      \item[global variables] declared outside of any function and remain in
        place throughout the execution of the entire program. they are stored at
        a fixed location in memory.
      \item[local variables] declared within a function and come into existence
        when the function is called and cease to exist when the function
        terminates. they are stored on the run-time stack.
    \end{description}
    ...
    \begin{figure}
      ...
      \begin{subfigure}[b]{.30\textwidth}
        ...
        \begin{tikzpicture}[
          ...
          % global variables
          \foreach [count=\i,evaluate=\i as \y using 1.5*(\i-1)] \l in {j,ch}
            \node[var,label=left:\l]  at (0,\y) {};
        \end{tikzpicture}
      ...
      \begin{subfigure}[b]{.30\textwidth}
        ...
        \begin{tikzpicture}[
          ...
          % stack variables
          \foreach [count=\i,evaluate=\i as \y using 1.5*(\i-1)] \l in {retVal,retAddr}
            \node[var,label=right:\l]  at (0,\y) {};

          % the shadow for the run-time stack
          \draw (-0.4,0.0) -- (3.4,0.0);
          \fill[pattern=north east lines] (-0.4,-0.5) rectangle ++(3.8,0.5);
        \end{tikzpicture}
